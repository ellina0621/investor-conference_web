...
def parse_report_period(title, announce_roc_year):
    ...
    # 「公告…財務報告董事會(預計)召開日期 / 董事會審議…預計召開」是預告開會日期，非實際通過 → 排除
    # 真正通過的主旨一定有「業經…通過」；預告型（含「預計通過/決議…召開日期」）沒有「業經」
    if re.search(r'召開日期|預計召開|擬召開|預定召開', t) and '業經' not in t:
        return (None, None)
...
def main():
    ...
    # 法說清單（每季 時段×主辦 各第一場）；網站用版去掉擇要以控制體積
    def _slim_sessions(lst):
        if not isinstance(lst, list): return []
        return [{k: s.get(k) for k in ('日期', '時間', '時段', '主辦', '類型')} for s in lst]
